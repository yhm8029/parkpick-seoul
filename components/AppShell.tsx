...
export function AppShell() {
  ...
  const collapseResults = useCallback((items: RecommendationResponse["recommendations"]) => {
    setResultsExpanded(false);
    setActiveId((current) => {
      const visible = items.slice(0, INITIAL_VISIBLE_RECOMMENDATIONS);
      return current && visible.some((item) => item.id === current)
        ? current
        : visible[0]?.id ?? null;
    });
  }, []);
  ...
  const beginEdit = useCallback(() => {
    cancelInFlight();
    collapseResults(result?.recommendations ?? []);
    setEditing(true);
    setError(null);
  ...
  const visibleRecommendations = useMemo(
    () => result
      ? result.recommendations.slice(
          0,
          resultsExpanded ? result.recommendations.length : INITIAL_VISIBLE_RECOMMENDATIONS,
        )
      : [],
  ...
  const mapRecommendations = result ? visibleRecommendations : [];
  ...
  const toggleExpanded = useCallback(() => {
    if (resultsExpanded) {
      collapseResults(recommendations);
    } else {
      setResultsExpanded(true);
    }
  }, [collapseResults, recommendations, resultsExpanded]);
