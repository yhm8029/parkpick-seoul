...
export function AppShell() {
  ...
  return <main>
    ...
    <section className="planner"><div className={`container planner-grid${result ? ` planner-grid--results planner-grid--mobile-${mobileView}` : ""}`}>{result ? <RecommendationPanel headingRef={resultHeadingRef} result={result} activeId={activeId} mobileView={mobileView} onSelect={activate} onMobileViewChange={setMobileView} onEdit={invalidateResult} /> : <div className="control-card"><div className="control-head"><div><span className="eyebrow">PARKING PLANNER</span><h2>방문 계획 입력</h2></div><Button variant="ghost" size="sm" onClick={demo}><RefreshCw size={15} /> 예시 채우기</Button></div>
