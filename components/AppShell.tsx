...
export function AppShell() {
  ...
  const [draftRevision, setDraftRevision] = useState<DraftRevision>(0);
  ...
  const useGps = useCallback(() => { if (gpsPlace) { cancelInFlight(); setOrigin(gpsPlace); } }, [gpsPlace, cancelInFlight]);
  ...
  return <main>
    ...
    <section className="planner"><div className={`container planner-grid${result ? ` planner-grid--results planner-grid--mobile-${result && !editing ? mobileView : "map"}` : ""}`}>{showForm ? <div className="control-card"><div className="control-head"><div><span className="eyebrow">PARKING PLANNER</span><h2>방문 계획 입력</h2></div><Button variant="ghost" size="sm" onClick={demo}><RefreshCw size={15} /> 예시 채우기</Button></div>
      <section className="step"><div className="step-title"><b>1</b><span><strong>출발지</strong><small>현재 위치 또는 직접 검색</small></span></div><div className={`gps-box${origin?.source === "GPS" ? " is-selected" : ""}`}><span className="gps-icon"><LocateFixed /></span><div><strong>내 위치로 출발</strong><p>{geo.error || (geo.status === "requesting" || geo.status === "checking" ? "GPS를 확인하고 있습니다." : geo.value ? `현재 위치 확인 완료 · ±${geo.value.accuracyMeters}m` : "버튼을 눌러 위치 권한을 요청합니다.")}</p></div>{geo.value ? <><Button variant="soft" size="sm" onClick={useGps}>{origin?.source === "GPS" ? "사용 중" : "출발지로 사용"}</Button><Button variant="ghost" size="icon" onClick={() => { onInputChange(); geo.refreshPosition(); }} aria-label="위치 새로고침"><RefreshCw size={17} /></Button></> : <Button variant="secondary" size="sm" onClick={() => { onInputChange(); geo.requestPosition(); }} disabled={geo.status === "requesting" || geo.status === "checking" || geo.status === "unsupported" || geo.status === "insecure"}>{geo.status === "requesting" || geo.status === "checking" ? <LoaderCircle className="spin" size={17} /> : <LocateFixed size={17} />} 현재 위치 사용</Button>}</div><div className="divider"><span>또는</span></div><PlaceSearch label="출발지 직접 검색" placeholder="예: 서울역, 강남구청" selected={origin} onSelect={selectOrigin} onClear={clearOrigin} hint="GPS 권한이 없어도 사용할 수 있습니다." /></section>
      <section className="step"><div className="step-title"><b>2</b><span><strong>목적지</strong><small>최종 방문 장소</small></span></div><PlaceSearch label="목적지 검색" placeholder="예: 코엑스, 더현대 서울" selected={destination} onSelect={selectDestination} onClear={clearDestination} /><div className="quick-places">{DEMO_PLACES.slice(0, 5).map(place => <button type="button" key={place.id} onClick={() => selectDestination(place)}>{place.name}</button>)}</div></section>
      <section className="step step--last"><div className="step-title"><b>3</b><span><strong>추천 조건</strong><small>체류시간과 우선순위</small></span></div><div className="options"><div className="options-title"><SlidersHorizontal size={17} /><strong>방문 조건</strong></div><div className="option-grid"><label>도착 예정<div className="inline"><input type="datetime-local" value={arrival} onChange={event => { onInputChange(); setArrival(event.target.value); }} /><Button variant="soft" size="sm" onClick={() => { onInputChange(); setArrival(toLocalDateTimeInput(new Date())); }}>지금</Button></div></label><label>예상 체류<select value={duration} onChange={event => { onInputChange(); setDuration(Number(event.target.value)); }}><option value={60}>1시간</option><option value={120}>2시간</option><option value={180}>3시간</option><option value={240}>4시간</option><option value={360}>6시간</option></select></label></div><fieldset><legend>추천 기준</legend><div className="profiles">{profileItems.map(item => <label key={item.value} className={profile === item.value ? "is-selected" : ""}><input type="radio" name="profile" checked={profile === item.value} onChange={() => { onInputChange(); setProfile(item.value); }} /><strong>{item.label}</strong><small>{item.sub}</small></label>)}</div></fieldset><div className="distance"><div className="distance-toggle" role="group" aria-label="탐색 반경"><button type="button" className={distanceMode === "AUTO" ? "is-selected" : ""} onClick={() => onSwitchMode("AUTO")}>AUTO</button><button type="button" className={distanceMode === "MANUAL" ? "is-selected" : ""} onClick={() => onSwitchMode("MANUAL")}>MANUAL</button></div>{distanceMode === "AUTO" ? <p className="distance-hint">가까운 공영주차장 3곳 자동 탐색</p> : <label className="distance-slider"><span>최대 거리 <strong>{manualDistance}m</strong></span><input type="range" min={50} max={1_000} step={50} value={manualDistance} onChange={event => onSliderChange(Number(event.target.value))} aria-label="최대 거리" /></label>}</div></div></section>
...
</div> : null}{result && appliedOrigin && !editing ? <RecommendationPanel headingRef={resultHeadingRef} result={result} activeId={activeId} mobileView={mobileView} onSelect={activate} onMobileViewChange={setMobileView} onEdit={beginEdit} appliedOrigin={appliedOrigin} onCancel={revertDraft} /> : null}<div className="preview-column"><MapPanel origin={mapOrigin} destination={mapDestination} recommendations={mapRecommendations} activeId={mapActiveId} onSelect={activate} />{result && appliedOrigin && !editing && active ? <div className="active-route"><div><small>{active.rank}순위</small><strong>{active.name}</strong><span>자동차 {active.driveMinutes}분 · 도보 {active.walkMinutes}분</span></div><NavigationButtons origin={appliedOrigin} parking={active} compact /></div> : null}<div className="route-summary"><div><Route /><span><strong>{(result ? mapOrigin?.name : origin?.name) || "출발지 미선택"}</strong><small>출발</small></span></div><ArrowRight /><div><Map /><span><strong>{mapDestination?.name || "목적지 미선택"}</strong><small>도착</small></span></div></div>{!result ? <div className="principles"><div><BadgeCheck /><span><strong>위치 최소 사용</strong><small>GPS 좌표를 저장하지 않음</small></span></div><div><Database /><span><strong>데이터 상태 표시</strong><small>실시간·지연·데모 구분</small></span></div><div><CarFront /><span><strong>네비 앱 위임</strong><small>카카오·네이버 연결</small></span></div></div> : null}</div></div></section>
