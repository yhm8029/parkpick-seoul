...
export function ParkingCard({ origin, parking, active, onSelect }: { origin: Place; parking: ParkingRecommendation; active: boolean; onSelect: () => void }) {
  ...
  return <article className={`parking-card${active ? " is-active" : ""}`}>
    <div className="parking-head"><button type="button" className="rank" onClick={onSelect}><strong>{parking.rank}</strong><span>순위</span></button><div className="parking-identity"><div><Badge tone={risk[1]}>{risk[0]}</Badge><Badge tone={status[1]}>{status[0]}</Badge>{parking.publicParkingType === "BP" ? <Badge tone="info">공공시설 부설</Badge> : null}{parking.source === "DEMO" ? <Badge tone="demo">예시 데이터</Badge> : null}</div><h3>{parking.name}</h3><p><MapPin size={14} /> {parking.address}</p></div><button type="button" className="score" onClick={onSelect}><strong>{parking.score}</strong><span>점</span></button></div>
