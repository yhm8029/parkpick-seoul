...
import { formatCurrency, formatDailyMaximumFeeLabel, formatFeeRateLabel } from "@/lib/utils";
...
export function ParkingCard({ origin, parking, active, onSelect }: { origin: Place; parking: ParkingRecommendation; active: boolean; onSelect: () => void }) {
  ...
  return <article className={`parking-card${active ? " is-active" : ""}`}>
    <div className="parking-head"><button type="button" className="rank" onClick={onSelect}><strong>{parking.rank}</strong><span>순위</span></button><div className="parking-identity"><div><Badge tone={risk[1]}>{risk[0]}</Badge><Badge tone={status[1]}>{status[0]}</Badge>{parking.publicParkingType === "BP" ? <Badge tone="info">공공시설 부설</Badge> : null}{parking.source === "DEMO" ? <Badge tone="demo">예시 데이터</Badge> : null}</div><h3>{parking.name}</h3><p><MapPin size={14} /> {parking.address}</p></div></div>
    ...
    <dl className="metrics"><div><dt><CarFront size={16} /> 주차장까지 자동차</dt><dd>{parking.driveMinutes}분</dd><small>{parking.routeSource === "ESTIMATE" ? "출발지 → 주차장 · 거리 기반 추정" : "출발지 → 주차장 · 현재 교통 기준"}</small></div><div><dt><Footprints size={16} /> 목적지까지 도보</dt><dd>{parking.walkMinutes}분</dd><small>주차장 → 목적지 · 약 {parking.walkDistanceMeters}m</small></div><div><dt><WalletCards size={16} /> 예상요금</dt><dd>{formatCurrency(parking.estimatedFee)}</dd><small className="fee-detail"><span>{formatFeeRateLabel(parking.feeRule)}</span>{dailyMaximumFeeLabel ? <span>{dailyMaximumFeeLabel}</span> : null}</small></div></dl>
    {!parking.realtimeSupported ? <p className="realtime-source-note">서울 주차 포털에서 이 주차장의 실시간 빈자리를 제공하지 않습니다.</p> : null}
    ...
    <NavigationButtons origin={origin} parking={parking} />
  </article>;
}
