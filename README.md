...
목적지만 정하면 서울·경기 공영주차장 후보를 비교하고, 현재 위치에서 선택한 주차장까지의 NAVER 교통 경로를 보여주는 모바일 우선 PWA입니다. 서비스 이름은 계속 `ParkPick Seoul`을 사용합니다.
...
3. 최대 10곳의 추천 중 먼저 표시되는 상위 3곳을 비교하고, 필요하면 `더 보기`로 나머지 후보와 지도 마커를 펼칩니다.
...
- 경기도 교통정보센터의 공영주차장 기본·실시간 정보 결합. 실시간 미지원 주차장도 기본정보 후보로 유지
- 서울·경기 API를 동시에 조회해 행정 경계 근처에서도 양쪽 후보를 거리 기준으로 비교
- 빈자리, 요금, 도보 거리, 자동차 이동시간, 최신성을 반영한 최대 10곳 추천과 상위 3곳 우선 표시
...
| 현재 빈자리       | 서울 주차 포털 또는 경기도 교통정보센터가 실시간 잔여면을 제공하는 주차장만 표시            |
...
교통시간은 요청 시점 교통상황에 따라 달라지며 네이버지도 앱에서 다시 조회한 값과 차이가 날 수 있습니다. 추천 결과는 주차 가능을 보장하지 않습니다. 추천 한 번에 NAVER Directions 5를 최대 10회 호출하므로 운영 환경에서는 NAVER Cloud 사용량 한도와 알림을 함께 관리합니다.
...
브라우저에서 <http://127.0.0.1:3000>을 엽니다. NAVER 키가 없으면 장소검색은 데모, 지도는 미리보기로 폴백합니다. 서울 주차 포털의 기본 근접 검색은 별도 키 없이 동작하지만, 포털 장애 시 대체 소스를 쓰려면 `SEOUL_OPEN_API_KEY`가 필요합니다. 경기 후보에는 승인된 `GYEONGGI_GITS_API_KEY`가 필요하며, 승인 대기 중에는 서울 결과만 정상 제공됩니다.
...
| `GYEONGGI_GITS_API_KEY`            | 서버 전용          | 경기도 주차장 기본·실시간 정보      |
